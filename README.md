...
**不依赖本地论文库**，每次分析时自动对 Semantic Scholar + arXiv 做大规模统计检索（按年份分桶），获取领域级论文总量和样本，再结合 LLM 做趋势分析：
- Semantic Scholar `total` 字段提供**领域级发表总量**（而非仅本地 10 篇样本）
- 每年份拉取 50 篇样本用于主题/方法分析
- 聚焦近两年，数据更具时效性
