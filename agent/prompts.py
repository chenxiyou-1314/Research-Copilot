...
METHOD_GRAPH_BUILD = """你是一个科研方法图谱分析师。请根据以下论文的方法解构结果，构建方法组件关系图。

方法解构结果:
{decomposition}

请构建方法组件关系图，节点 = 方法组件，边 = 相似/关联关系。输出JSON格式:
...
{{
    "nodes": [
        ...
        {{
            ...
            "type": "classifier",
            "papers": ["iVoro", "CEC"],
            "detail": "基于类原型的分类，避免全连接层参数膨胀"
        }}
    ],
    "edges": [
        {{
            "source": "resnet18",
            "target": "proto_classifier",
            "relation": "组合",
            "detail": "ResNet提取特征→原型分类器计算距离，是CIL经典搭配"
        }},
        {{
            "source": "proto_classifier",
            "target": "voronoi_proto",
            "relation": "改进",
            "detail": "Voronoi原型是对普通原型的几何增强"
        }}
    ]
}}
```
...
要求:
- nodes: 每个方法组件是一个节点，id 用英文小写+下划线，name 用可读名称，type 是组件类别（backbone/classifier/loss/augmentation/training_strategy/evaluation等）
- nodes.papers: 列出使用了该组件的论文
- edges: 关系类型包括：
  - "相似"：功能相近的不同方法（如不同backbone、不同loss变体）
  - "改进"：后者是前者的改进/变体
  - "组合"：经常一起使用的方法对
  - "互补"：解决不同子问题但可协同的方法
- 边的数量不必太多，5-12条有意义的边即可，不要强行建边
- 节点数量 8-20 个，覆盖主要方法组件
- 所有分析必须基于给定的解构数据，不要编造不存在的组件或论文"""
